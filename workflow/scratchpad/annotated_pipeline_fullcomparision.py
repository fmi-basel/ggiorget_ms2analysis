...
for i, (pos, mean, sd, color) in enumerate(
        zip(category_positions[1:-1], df_correlation['corr_off'], df_correlation['sd_off'], colors_gregory)):
...
color_id = 0
kmf.fit(
    (df_km_times[(df_km_times['detectiontype'] == detection_type) & (df_km_times['clone'] == clone)]['framecount'] / 2),
    event_observed=df_km_times[(df_km_times['detectiontype'] == detection_type) & (df_km_times['clone'] == clone)][
        'censored'],
    label=clone)
km_times = kmf.survival_function_.index.values
km_survival_probs = kmf.survival_function_[clone].values
